	...
	if (nVMs == 0)
    {
        ...
        for (jsize i = 0; i < nVMs; i++)
        {
            ...
            for (jint i = 0; i < loadedClassesCount; i++)
            {
                ...
                if (!strcmp(signature, "Lnet/minecraft/launchwrapper/Launch;"))
                {
                    clsNmlLaunch = loadedClasses[i];
                }
            }

            //clsNmlLaunchClsLoader = jniEnv->FindClass("net/minecraft/launchwrapper/LaunchClassLoader");
            jfieldID test1 = jniEnv->GetStaticFieldID(clsNmlLaunch, "classLoader", "Lnet/minecraft/launchwrapper/LaunchClassLoader;");

            objNmlClassLoader = jniEnv->GetStaticObjectField(clsNmlLaunch, test1);
